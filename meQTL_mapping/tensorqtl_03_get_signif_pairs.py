...
### get all significant phenotype-variant pairs in cis-QTL permutation mode 

import sys
import os
import glob

chr_files = {os.path.basename(f).split('.')[2]:f for f in glob.glob(f'{outdir}/{prefix}.cis_qtl_pairs.*.parquet')}  # outputs from map_nominal
...
signif_df = post.get_significant_pairs(cis_df2, chr_files, fdr=0.05)

# save
signif_df.to_csv('/exports/cmvm/eddie/smgphs/groups/Quantgen/Users/vasilis/PHD/EBB_methylation/tensorqtl/results/EBB.cis_qtl_signif_pairs.csv')
